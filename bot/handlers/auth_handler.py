"""
Auth Handler v2 - SafeLinkU verification dengan token acak.
"""
import logging
from pyrogram import Client, filters
from pyrogram.types import CallbackQuery, InlineKeyboardMarkup, InlineKeyboardButton
from bot.utils.auth import verify_user, is_verified
from bot.utils.safelinku import generate_token, validate_token, get_verify_safelink_text, get_safelink_url
...
class AuthHandler:
    ...
    async def verify_callback(client: Client, callback: CallbackQuery):
        ...
        safelink_url = get_safelink_url(token)
        
        await callback.answer("Link verifikasi dibuat!", show_alert=False)
        
        try:
            await callback.message.edit_text(
                get_verify_safelink_text(token),
