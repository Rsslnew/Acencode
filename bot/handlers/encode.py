...
async def handle_video(client: Client, message: Message):
    ...
    if not await queue_manager.acquire_slot(user_id):
        await message.reply(
            f"Queue full!\nYou have {Config.MAX_QUEUE_PER_USER} active jobs."
        )
        try:
            await message.delete()
        except Exception:
    ...
    try:
        await _encode_pipeline(client, message, tracker, queue_msg, settings)
    except Exception as e:
...
async def handle_video_doc(client: Client, message: Message):
    ...
    logger.info(f"VIDEO_DOC_HANDLER: user={user_id} chat={message.chat.id}")
    ...
    if not await queue_manager.acquire_slot(user_id):
        await message.reply(
            f"Queue full! You have {Config.MAX_QUEUE_PER_USER} active jobs."
        )
        try:
            await message.delete()
        except Exception:
...
async def _encode_pipeline(client, message, tracker, queue_msg, settings):
    user_id = message.from_user.id
    username = message.from_user.username or message.from_user.first_name
